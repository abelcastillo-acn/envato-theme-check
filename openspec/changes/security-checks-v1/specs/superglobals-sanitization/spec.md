...
### Requirement: Blanket $_SERVER usage warning (class-envato-check.php)
**Reason**: The existing Envato rule `'/[^a-z0-9](?<!_)\$_SERVER\s?/'` ("PHP Global Variable found. Ensure the context is safe and reliable") fires on every `$_SERVER` read including safe keys such as `REQUEST_METHOD`, and — because that check fails the run on WARNING — effectively acts as REQUIRED. It is superseded by the key-aware, sanitizer-aware rule above.
**Migration**: Remove the rule from `checks/class-envato-check.php`; reviewers get the same coverage from `superglobals/unsanitized` with fewer false positives. Document in readme.md.
